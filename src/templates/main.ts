...
export function mainCommentTemplate({
  ...
  const linkGen = (endpoint: Endpoint): string => {
    if (baseBatchCommit) {
      return `${CLOUD_SPEC_VIEWER_BASE}/people/${personId}/public-specs/${specId}/changes-since/${baseBatchCommit}/paths/${endpoint.pathId}/methods/${endpoint.method}`
    } else {
      return `${CLOUD_SPEC_VIEWER_BASE}/people/${personId}/public-specs/${specId}/documentation/paths/${endpoint.pathId}/methods/${endpoint.method}`
    }
  }
  ...
  const specUrl = `${CLOUD_SPEC_VIEWER_BASE}/people/${personId}/public-specs/${specId}/${
